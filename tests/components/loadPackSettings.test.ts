...
describe('load pack placement', () => {
  ...
  it('scans ComfyUI custom_nodes and imports builder-owned packs from Settings', async () => {
    const fetchMock = vi.fn().mockResolvedValue(new Response(JSON.stringify({
      packs: [
        {
          packName: 'AudioTools',
          path: '/ComfyUI/custom_nodes/AudioTools/builder.project.json',
          project: {
            name: 'Audio Tools',
    ...
    expect(emitted?.[0]).toEqual([
      ...
      'AudioTools',
      '/ComfyUI/custom_nodes/AudioTools/builder.project.json',
    ])
  })
  ...
  it('renames the active pack display name and ComfyUI folder from Settings', async () => {
    ...
    await wrapper.get('[data-testid="settings-pack-name"]').setValue('Vision Tools')
    await wrapper.get('[data-testid="settings-pack-name"]').trigger('change')
    ...
    expect(wrapper.text()).toContain('ComfyUI/custom_nodes/Vision_Tools')
  })
})
